...
public class HostUtil {
    ...
    private static String getHostFile() {
        String fileName;
        // 判断系统
        if ("windowns".equalsIgnoreCase(System.getProperty("os.name"))) {
    ...
    public synchronized static boolean updateHost(String ip, String domain) {
        ...
        try {
            hostFileDataLines = FileUtils.readLines(new File(fileName), Charset.defaultCharset());
        } catch (IOException e) {
            System.out.println("Reading host file occurs error: " + e.getMessage());
            return false;
        }
        ...
        for (Object line : hostFileDataLines) {
            String strLine = line.toString();
            // 将host文件中的空行或无效行，直接去掉
            if (StringUtils.isEmpty(strLine) || "#".equals(strLine.trim())) {
                continue;
            }
         ...
         */
        if (!findFlag) {
            newLinesList.add(ip + splitter + domain);
        }
        /**
         * Step4: 不管三七二十一，写设定文件
         */
    ...
    public static void main(String[] args) {
        updateHost("172.28.4.152", "service-corp.odps.aliyun-inc.com");
    }
